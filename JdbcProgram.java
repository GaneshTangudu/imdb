...
public class JdbcProgram {
	...
	public static void main(String[] args) 
	{
		...
		while(r>=0)
		{
			...
			case 1:
				...
				break;
			case 2:
				System.out.println("How many actors want to add 1/2");
				int z=sc.nextInt();
				if(z==1)
				{
				i.addMovie();
	...
	public void addActor() 
	{
		try {
			Connection con=getConnect();
			String query="insert into actors(actorno,name,dob) values(seq_actors.nextval,?,?)";
			PreparedStatement pst=con.prepareStatement(query);
			System.out.println("Name :");
			String name=sc.nextLine();
	...
	public void addProducer()
	{
		try {
			Connection con=getConnect();
			String query="insert into producers(producerno,name,dob) values(seq_producers.nextval,?,?)";
			PreparedStatement pst=con.prepareStatement(query);
			System.out.println("Name :");
			String name=sc.nextLine();
			System.out.println("Dob(DD-MM-YYYY) :");
	...
	public void actors()
	{
		try {
			Connection con=getConnect();
			PreparedStatement pst=con.prepareStatement("SELECT * FROM Actors");
			ResultSet rs=pst.executeQuery();
			while(rs.next())
			{
		...
		public void producers()
    {
    	try {
			Connection con=getConnect();
			PreparedStatement pst=con.prepareStatement("SELECT * FROM PRODUCERS");
			ResultSet rs=pst.executeQuery();
			while(rs.next())
			{
				 System.out.print(rs.getString(1)+"."+rs.getString(2)+"  ");
    ...
    public void getActor(int a,int b,int c)
    {
    	try {
			Connection con=getConnect();
			PreparedStatement pst1=con.prepareStatement("select name from actors where actorno in(select actor1 from imdb where actor1=?)");
			PreparedStatement pst=con.prepareStatement("select name from actors where actorno in(select actor2 from imdb where actor2=?)");
			PreparedStatement pst2=con.prepareStatement("select name from producers where producerno in(select producer from imdb where producer=?)");
			pst1.setInt(1,a);
			pst.setInt(1,b);
			pst2.setInt(1,c);
